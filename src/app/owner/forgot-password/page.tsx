...
export default function OwnerForgotPasswordPage() {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    if (cleanEmail !== 'emc.egypt12@gmail.com') {
      setErrorMsg('البريد الإلكتروني المدخل غير مطابق للبريد المسجل للمالك.');
      return;
    }
  ...
  const handleVerifyAndReset = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: 'emc.egypt12@gmail.com', code: code.trim(), action: 'verify' }),
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'الرمز المدخل غير صحيح');

      // Update owner bypass password
      localStorage.setItem('emc_owner_password', newPassword.trim());

      setSuccessMsg('تم تحديث كلمة مرور المالك بنجاح! جاري تحويلك...');
      setTimeout(() => {
    ...
    <div className="min-h-screen bg-[#F8F9FA] text-[#1A1A2E] flex items-center justify-center p-4 relative overflow-hidden text-start">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl font-extrabold tracking-tight text-[#1A1A2E] font-outfit">استعادة كلمة مرور المالك</h2>
          <p className="text-xs text-gray-500 mt-2">أدخل بريد المالك للحصول على رمز تحقق مؤقت</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {errorMsg && (
            <motion.div 
              ...
              className="mb-5 p-4 bg-red-50 border border-red-200 text-red-700 text-xs font-semibold rounded-xl flex items-center gap-2.5 text-right" 
              dir="rtl"
            >
          ...
          {successMsg && (
            <motion.div 
              ...
              animate={{ opacity: 1, height: 'auto' }}
              exit={{ opacity: 0, height: 0 }}
              className="mb-5 p-4 bg-emerald-50 border border-emerald-250 text-emerald-800 text-xs font-semibold rounded-xl flex items-center gap-2.5 text-right" 
              dir="rtl"
            >
          ...
          {step === 'request' ? (
            ...
            >
              <div className="space-y-1.5 text-right">
                <label className="text-xs font-bold text-gray-650">بريد المالك المسجل</label>
            ...
            >
              ...
              <div className="space-y-1.5 text-right">
                <label className="text-xs font-bold text-gray-650">كلمة المرور الجديدة للمالك</label>
                <div className="relative">
        ...
        <div className="mt-6 pt-4 border-t border-gray-100 text-center">
          <a href="/owner/login" className="text-xs text-gray-500 hover:text-primary inline-flex items-center gap-1 transition-colors">
